...
| 27 | **R-1's added challenger instruction (use the injected core as the cold standard, hold memory for warm) is reassured, not proven.** The only evidence is a fail-only probe — a pass is expected regardless. | Medium | Accepted (M7) — the thinnest T2 corner; direction-b untested |
| 28 | **The honest block's standard is a runtime-injection dependency** — presence-guarded, but a *variable* standard across runtimes/versions. | Low | Accepted (M7) — varying-but-honest beats fixed-but-false |
| 29 | **Axis-1 over-claim completeness is a bounded, single-party semantic review.** No complete automated guard exists for the class (that would be the forbidden open blacklist). | Low | Accepted (M7) — bounded, not closed |
